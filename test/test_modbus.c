...
requerida y es usada dentro del mensaje a ser devuleto por el protocolo Modbus. En este caso se
testea coils, discrete, holding e input. Las dos primeras son pedidos de bits que el programa tiene
que trabajar a nivel de bytes, por eso la conversión. En el caso de las dos últimas se consulta la
dirección de un byte, pero la respuesta será de dos bytes que es donde se aloja el dato completo.
